...
If we assign the vibrational-state excitations to the equispaced parallel conductance lines within a Coulomb diamond, we might be able to link the Franck--Condon factor to the \gls{DC} conductance maxima. Thus, by extracting and averaging within a  bias window parallel lines to the Coulomb edges, we observed that the conductance peaks follows a known progression~\cite{leturcq2009franck}.
...
\end{table}
%
These findings appear to imply that, at the 95\% confidence level, we cannot rule out the possibility that the SET obtained when using the 3D bulky groups was produced by chance. However, a number of specifics are required. To begin, statistical validity exists only if the experiment is repeated under same conditions. However, in this case, the number of detected \glspl{SET} across various measured chips is determined by the total number of devices surviving the thermal shock from \SI{50}{\kelvin} to cryogenic temperature. This does not rule out the possibility that more \glspl{SET} were generated but could not be measured because of this secondary process interfering with our statistics.
...
\section{Discussion}\label{solubility:sec:summary}
A close inspection of the low-temperature Coulomb oscillations has revealed the presence of conductance suppression for small $eV_\textrm{SD}$, which we explained within the Franck--Condon conductance-blockade mechansim.
...
The presence of sterically-hindred chemical functionalisations grated onto the edges has enabled \glspl{GNRSET} where vibrational states can be probed through transport spectroscopy, revealing level of details comparable with ultra-clean \gls{CNT} embedded in a \gls{SET}~\cite{sapmaz2005electronic,sapmaz2006tunneling}. Investigations of mechanical and quantum vibrations are paramount when excitations in the \si{\mega\hertz}--\si{\giga\hertz} range are sent to excite and manipulate spins~\cite{vincent2012electronic, gaita2019molecular, escalera2018spin}. We therefore consider this work a starting point in that direction, which might lead to the first the manipulation of qubits embedded in all-carbon molecular structure, similar to those proven in semiconductor quantum dots~\cite{hanson2007spins, burkard2021semiconductor}.

By reducing the lattice vibrations by rationally design \gls{MGNR}, one could, in principle, increase the spin--phonon coupling, leading to precited long coherence times~\cite{palyi2012spin}. This platform could be realised, for example, by either grafting onto the edges porphyrin rings where the metal ion is a single-molecule magnets, or direct substitutional inclusion within the graphene lattice.

Theoretical and numerical estimates have also suggested several reasons why \glspl{MGNR} may be an appealing class for spintronic devices. In particular, spin--relaxation time between \SIrange{10}{1000}{\milli\second} have been predicted, when the dechorence mechanisms are limited to only single-phonon processes~\cite{droth2013electron}. Albeit these predictions are very encouraging, experimental evidences reported that a single-phonon process is unrealistic and that two-phonon processes should also be studied, which may be lead to lower spin--lattice relaxation times~\cite{slota2018magnetic}. Moreover, spin-diffusion-length calculations suggested values of approximately $\SI{17}{\micro\metre}$ in presence of an extrinsic spin--orbit coupling, and that hot-spot processes could futher increase these values~\cite{rojas2019ab}. Both spin-lattice relaxation times and spin-diffusion lengths have been linked to the $l_\textrm{GNR}/w_\textrm{GNR}$ ratio, suggesting increments for larger ratios. Even though it cannot be excluded that longer \glspl{GNR} will pose more questions, we believe atomically-precise long ribbons ($l_\textrm{GNR}\ge\SI{100}{\nano\metre}$) could be a suitable platform for addressing all these challenges.

\section{Project Overview}
\glspl{MGNR}\textbf{1} were supplied by the group of Akimitsu Narita, whereas \glspl{MGNR}\textbf{2} were synthesised in Dresden by the group of Xinliang Feng. The Franck--Condon analysis was undertaken by Dr Simen Sopp (see also~\cite{sopp2021phd}), whereas I acquired temperature data, and Fanmiao Kong calculated the phonon dispersion. Dr Alex Gee supported me in fabricating the devices when I was compelled to stay out of the lab for more than a month owing to covid-19 infection and subsequent health issues that necessitated medical care.