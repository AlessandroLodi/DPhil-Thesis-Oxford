...
\begin{table}[]
    ...
    \begin{tabular}{ccccccccc}
        ...
        \midrule
        Ohmic             & \SI{1.41e-5}    & \SI{4.06e-10}    & \SI{3.4e4}                     & 29                    & 99.6       \\\T
        Partial S/D-doped & \SI{1.50e-5}    & \SI{2.33e-8}     & \SI{6.4e2}                     & 63.6                  & 93.1       \\\T
        Schottky-Barrier  & \SI{3.26e-6}    & \SI{2.31e-8}     & \SI{1.41e2}                    & 19.6                  & 109.8      \\\T
        Tunnel            & \SI{5.53e-6}    & \SI{1.16e-8}     & \SI{4.76e2}                    & 13                    & 107.9      \\
        \bottomrule
    \end{tabular}
\end{table}
\newpage
...
Two years after their breakthrough, \Citeauthor{Franklin2012} shrank the channel length of a \gls{CNTFET} to less than 10 nanometers~\cite{Franklin2012} (Fig.~\ref{fig:intro:cnt_device_2}). Their technique involved the use of smaller tubes and a reduction of the necessary \gls{EOT} to \SI{0.65}{\nano\metre}. Even by adopting a local gate, if the gate oxide is excessively thick the gate tends to lose control over the flow of carriers in the channel~\cite{sze2021physics}.
