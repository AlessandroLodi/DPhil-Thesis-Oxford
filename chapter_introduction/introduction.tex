...
\begin{table}[]
    ...
    \begin{tabular}{ccccccccc}
        \toprule
        \textit{Contact}  & $I_\textrm{on}$     & $I_\textrm{off}$     & $I_\textrm{on}/I_\textrm{off}$ & $g_\textrm{mt}$       & $SS$     & \\
        \textit{type}     & (A)                 & (A)                  &                                & (\si{\micro\siemens}) & (mV/dec) & \\
        \midrule
        Ohmic             & 1.41$\times10^{-5}$ & 4.06$\times10^{-10}$ & 3.4$\times10^{4}$              & 29                    & 99.6       \\\T
        Partial S/D-doped & 1.50$\times10^{-5}$ & 2.33$\times10^{-8}$  & 6.4$\times10^{2}$              & 63.6                  & 93.1       \\\T
        Schottky-Barrier  & 3.26$\times10^{-6}$ & 2.31$\times10^{-8}$  & 1.41$\times10^{2}$             & 19.6                  & 109.8      \\\T
        Tunnel            & 5.53$\times10^{-6}$ & 1.16$\times10^{-8}$  & 4.76$\times10^{2}$             & 13                    & 107.9      \\
        \bottomrule
    \end{tabular}
\end{table}
\newpage
...
\begin{figure}[hbtp]
    \begin{center}
        ...
        \caption[High-$\kappa$ Carbon Nanotube Field-Effect Transistors]{
            \textbf{a}, Lanthanum oxide is a high-$\kappa$ dielectric that is thermally stable when placed in contact with Si. With these devices it has been possible to reach subthreshold swing close to the theoretical limit of $SS=\SI{60}{\milli\volt}$~dec$^{-1}$~\cite{franklin2013consistently}.
...
In \glspl{CNTFET}, two are the main device parameters that can be scaled down: the contact length $L_\textrm{c}$ and the channel length $L_\textrm{ch}$. Aggressive $L_\textrm{ch}$ scaling typically reduces the gate control over the channel, hence the ability of the electric field generated through the gate to bend the silicon band structure. Intuitively, as the channel length is reduced, the electric field created by the source contact will be felt by the charge carriers more and more. \Glspl{CNT} may provide a suitable platform for such scaling because of their ultrathin, hollow nature. This could be, however, a double-edged sword. Since the electron effective mass is smaller\footnote[2]{$m^*_\textrm{CNT} = (0.228 \pm 0.129)m_0$~\cite{marulanda2008carrier}} compared to that of silicon, the tunneling probability may be large enough to increase drastically the source--drain current. From a device performance point of view, this often means an increase in the $SS$ and high $I_\textrm{off}$.

\Citeauthor{franklin2010length} conducted one of the earliest systematic studies on channel-length scaling effects in 2010, through a series of experiments on \glspl{CNT}~\cite{franklin2010length}. When the channel length was reduced by more than two orders of magnitude (from \SI{3}{\si{\micro\metre}} down to \SI{15}{\si{\nano\metre}}), a consistent trend was found in both the threshold voltage ($V_\textrm{Th}\simeq\SI[separate-uncertainty = true]{-0.5(0.2)}{V}$) and subthreshold swing ($SS\simeq\SI{90}{\milli\volt}$~dec$^{-1}$).
...
In addition, at that time part of the scientific community believed that scaling had little to no effect on the transconductance $g_m$ of the device~\cite{avouris2003carbon,javey2009carbon, anantram2006physics}. The transconductance of a conventional \gls{MOSFET} is the change in drain current divided by the amount that the gate-to-source voltage varies at a constant bias. Typical values lies in the range \qtyrange[range-units = bracket]{1}{30}{\milli\siemens\per\micro\metre} for a small-signal field-effect transistor.

For an n-channel \glspl{MOSFET} with a certain threshold voltage, the drain current at a given gate in the saturation regime ($V_\textrm{Th} > V_\textrm{G} - V_\textrm{SD}$) is defined:
%
\begin{equation}
...
It is possible to work out the value of $g_m$ by using the Shichman--Hodges model if we assume that the threshold voltage $V_\textrm{Th}$ is a device constant. Defining the overdrive voltage as $V_\textrm{ov} = V_\textrm{G} - V_\textrm{Th}$, we obtain:
%
\begin{equation}
    g_\textrm{m} = \pdv{I_\text{SD}}{V_\text{GS}} =\frac{2~I_\textrm{SD}}{V_\textrm{ov}}.\label{eq:schiman-hodges-model}
\end{equation}
%
The overdrive voltage is frequently used during the design phase of a transistor because it conveniently links the drain current with the applied bias: in fact, by rearranging Eq.~\ref{eq:transistor-drain-current} and identifying $k_n = \mu_n C_\textrm{ox}\frac{W}{L}$, we obtain:
%
\begin{equation}
    I_\textrm{SD} = \frac{1}{2}k_n V_\textrm{ov}^2 \implies V_\textrm{ov} = \sqrt{\frac{2I_\textrm{SD}}{k_n}}
\end{equation}
%
The advantage of this equation is that the relationship written in terms of $V_\textrm{ov}$ rather than $V_\textrm{Th}$ holds true for both n- and p-types of doping. Franklin and his colleagues produced transconductance values as high as \SI{40}{\micro\siemens\per\micro\metre} for a \gls{CNTFET} with a channel length of \SI{15}{\nano\metre}, which corresponds to transconductance values of approximately \qtyrange[range-units = bracket]{91}{110}{\milli\siemens\per\micro\metre}, thereby meeting the requirements for compatibility with Si technology.
...
Two years after their initial breakthrough, Franklin and his colleagues shrank the channel length of a \gls{CNTFET} to less than 10 nanometers~\cite{Franklin2012}. Their technique involved the use of smaller tubes and a reduction of the necessary \gls{EOT} to \SI{0.65}{\nano\metre}. Even by adopting a local gate, if the gate oxide is excessively thick the gate tends to lose control over the flow of carriers in the channel~\cite{sze2021physics}.
The length of the channel was successfully lowered from 320 to \SI{9}{\nano\metre}. The authors reported an on-current with values closer the the ballistic limit for transistors with channel lengths close to \SI{41}{\nano\metre}~\cite{franklin2010length}.

By examining the subthreshold swing $SS\simeq\SI{94}{\milli\volt}$~dec$^{-1}$, we can appreciate the greater control from the gate to the conductive channel, which at the time, exceeded that of any Si-based device. \citeauthor{franklin2010length} advocated for decreasing the \gls{CNT} diameter to reduce the subthreshold swing and off-current with minimum effect to the on-current.
The process of scaling down a \gls{CNTFET}, however, does not come without any cost: the contribution of carrier back injections to the drain conduction band grows with the decrease of the gate length. Typically, this is indicated by an increase of the minimum current. To avoid this unfavorable off-state, the nature of the junction contact and its scaling behaviour must be examined.
...
When \glspl{MOSFET} are shrank down to the nanoscale, one must consider also the contact lenght of the nanotube with source and drain, $L_\textrm{c}$. In particular, computational studies have shown that higher contact transparency is expected when the metal--\gls{CNT} coupling is weak~\cite{fediai2015electron, teichert2019improved, fediai2016towards}. Especially for long  contacts ($L_\textrm{c}\sim\SI{30}{\nano\metre}$), one conclusion we can take from these studies is that in order to achieve high contact transparency one should minimize coupling and maximize the contact length. With respect to Ti contacts, \citeauthor{liu2017scaling} suggested that the weaker Pd--\gls{CNT} coupling was responsible for almost Ohmic contacts~\cite{liu2017scaling}.

By using the transfer length model, the contact length was estimated to be inversely dependent on the contact resistance between the nanotube and the metallic pads~\cite{purewal2007scaling, franklin2010length}. These experimental observations were initially explained by assuming dipole moments building up at the metal--\gls{CNT}interface~\cite{leonard2000role}, which is likely caused by interface states at the junctions, formed by charges which provide a potential that decays within a few nanometers from the junction. A high density of dipole charge translates to a high density of interface states, which is expected to pin the Fermi level. Since in a planar contact, the dipole in a metal--\gls{CNT} contact is localised in all directions, the overall effect of these interface states is to shift the bands within a few nanometers of the junction, generating a narrow barrier, whilst the energy of the bands deeper within the CNT remain the same as for a contact without interface states~\cite{svensson2011schottky}. One possible conclusion is that by having an electrical contact which has a work function closer to the the nanotube might protect the metal--nanotube junction to undergo to Fermi level pinning effects caused by surface states.
