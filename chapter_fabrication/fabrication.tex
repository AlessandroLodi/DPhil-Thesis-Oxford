...
\section{Cryogenic-Temperature Transport Setup}\label{ch:fabrication:triton_setup}
...
\begin{figure}[hbtp]
    \begin{center}
        ...
        \caption{\textbf{a,} Hall voltage measured across one cross when the current driven is increased. The signal is proportional to the current as expected. The parameters $f$ and $\tau$ are the lock-in operation frequency and integration time, respectively. The frequency in particular was optimised by taking a frequency scan the line response and set it to the point of minimum noise. \textbf{b,} The Hall coefficient for this particular device was calcualted to be $R_\textrm{H}\sim \SI{1e-6}{\ohm}$G$^{-1}$ which is expected for an Au probe of $d \sim \SI{1}{\micro\metre}$}
